...
tokens = ["VAR", "IDENT:X", "ASSIGN", "NUMBER:5", "VAR", "IDENT:Y", "ASSIGN", "NUMBER:10", "PRINT", "IDENT:X", "MULT", "IDENT:Y", "EOF"]
#function foo(a){return foo}
tokens = ['FUNCTION', 'IDENT:foo', 'LPAREN', 'IDENT:a', 'RPAREN', 'LBRACE', 'PRINT', 'NUMBER:5', 'RETURN', 'IDENT:a', 'RBRACE', 'EOF']
...
def FunctionParams(token_index):
    ...
    if "RPAREN" == tokens[token_index]:
        return [True, token_index+1, ["FunctionParams1", tokens[token_index]]]
    return [False, token_index, []]
...
def MultipleAssignment(token_index):
    #size of NameList must = # of Function returns
...
def FunctionCallParams(token_index):
    ...
    print("false at functioncallparams")
    return [False, token_index, []]
...
if __name__ == '__main__':
    ...
    '''
    My admittedly sloppy way of getting just the tree out of the list
    of returned values.
    '''
    bigTree = (Program(0))
    smallTree=bigTree[2]
